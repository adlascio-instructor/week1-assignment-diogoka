...
Note
Not sure where to get started? It might be useful to start by creating a variable to keep track of how many dirty samples we find as we loop through the array of samples. 
We can increment or add to this number using the ++ operator. Once you know how many dirty samples there are, we just need to do some simple math to determine if it exceeds the threshold.
*/

const checkAir = function (samples, threshold) {
  let numberOfSamples = samples.length;
  let numberOfDirtySamples = samples.reduce((acc, curr) => {
    if (curr === "dirty") {
      return acc + 1;
    }
    return acc
  ...
  let percentageOfDirtySamples = numberOfDirtySamples / numberOfSamples;
  return percentageOfDirtySamples > threshold ? "Polluted" : "Clean";
};
