/*
Welcome back to Codeville! The citizens of Codeville seem pleased with all the upgrades your administration has been making to the local infrastructure, and they want more! 
...
It does not matter which spot your function chooses, as long as the spot is available. And if there are no available spots, remember to return false.
*/

const whereCanIPark = function (spots, vehicle) {
  const result = [];
  spots.forEach((element, row) => {
    element.forEach((e, column) => {
      if (vehicle === 'regular' && e === 'R' && result.length === 0) {
        result.push(column, row);
      }
      if (vehicle === 'small' && (e === 'R' || e === 'S') && result.length === 0) {
        result.push(column, row);
      }
      if (vehicle === 'motorcycle' && (e === 'R' || e === 'S' || e === 'M') && result.length === 0) {
        result.push(column, row);
      }
    })
  });
